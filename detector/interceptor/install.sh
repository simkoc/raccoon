...
echo "this script is deprecated! DO NOT USE NO MORE!"
exit 1

if [ $# -ne 7 ]; then
    echo "usage: ./install.sh </full/path/to/vm.ova> <vm-type> <vm-name> <vm-state> <user> <vm-ip> <pwd>"
    exit 1
fi
...
vboxmanage snapshot ${vmname} take virgin-state
vboxmanage controlvm ${vmname} poweroff


echo "vm ${vmname} successfully installed"
#echo "IP: ${ip}"
#echo "Credentials: ${vm_credentials}"
#echo "snapshot: virgin-state"
echo "rebooting vm now"
echo "waiting for 30sec"
sleep 30
...
if vboxmanage list vms | grep --quiet "\"${vmname}\""; then
    ...
    if vboxmanage list runningvms | grep --quiet "\"${vmname}\""; then
        echo "test vm ${vmname} is currently running - shut down before trying again with using die .vdi and polesno.sh"
        exit 1
    else
        echo "restoring snapshot ${vmstate}"
...
#waiting for machine to be accessible
echo "waiting for machine to connect to network... (90s)"
sleep 30
echo "waiting for machine to connect to network... (60s)"
sleep 30
echo "waiting for machine to connect to network... (30s)"
sleep 30
...
sudo sed -i "/LogFormat.* common/s/.*/&\nLogFormat %r ground_data_log/" "/opt/bitnami/apache2/conf/httpd.conf";
...
echo "creating new snapshot ${vmstate}-mysql-proxy"
vboxmanage snapshot $vmname take "${vmstate}-mysql-proxy"
