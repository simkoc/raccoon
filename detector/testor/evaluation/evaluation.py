...
def count_equalp_fingerprints(fingerprint, refFingerprints,
                              projname, session, user, db_host, db_user, db_pwd, db_name, expid, logger=None):

    with db_interface.get_connection(db_host, db_user, db_pwd, db_name) as db_con:
        logger.info("getting apache-logs from database")
        log1, log2 = db_interface.get_experiment_logs(db_con, expid, logger=logger)
        
    count = 0
    dmoracle = LogBasedParameterOracle(log1, log2, logger=logger)
