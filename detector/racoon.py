...
def parse_args(args):
    ...
    wt_parser.add_argument("root",
                           help="the root user name of the vm")
    wt_parser.add_argument("vm_ip",
                           help="the ip of the vm")
    wt_parser.add_argument("root_pwd",
    ...
    wt_parser.add_argument("firebases",
                           help="a csv list of the firebases to be used")
...
def single_suspect_test(pargs, logger):
    firebases = [firebase.split(":") for firebase
                 in pargs.firebases.split(",")]
    ...
    for id in ids:
        ...
        vulnerability_single(id, pargs.host, pargs.user,
                             ...
                             pargs.xpath, pargs.vm_ip, pargs.root,
...
def reeval_test_result(pargs, logger=None):
    graph = py2neo.Graph(host=pargs.neohost,
                         user=pargs.neouser,
                         password=pargs.neopwd)
    race_suspect = None
    ...
    if pargs.relaxed_check:
        count = count_equalp_fingerprints_relaxed(suspectFingerprint, race_suspect._query, fingerprints,
                                                  graph, race_suspect._projname, race_suspect._session,
                                                  race_suspect._user, logger=logger)
    else:
        count = count_equalp_fingerprints(suspectFingerprint, fingerprints, graph, race_suspect._projname,
                                          race_suspect._session, race_suspect._user, logger=logger)
...
def full(pargs, logger=None):
    ...
    firebases = [firebase.split(":") for firebase
                 in pargs.firebases.split(",")]
    ids = interleaving_single(pargs.expid, pargs.host, pargs.user,
    ...
    sequentialRefPrints, sequentialRunCount = get_litmus_test_results(pargs.expid,
                                                                      pargs.host,
                                                                      pargs.user,
                                                                      pargs.pwd,
                                                                      pargs.database,
