...
import argparse
import json
from detector.database.postgres import get_connection, store_experiments, store_sel_commands, get_highest_experiment_id, store_http_requests, store_xdebugs, get_all_http_requests
...
class xdebugs:
    def __init__(self, expid, selcmdctr, httpreqctr, name, content):
        self._expid = expid
        self._selcmdctr = int(selcmdctr)
        self._httpreqctr = int(httpreqctr)
        self._name = str(name)
        self._content = zlib.compress(open(content).read())
...
def create_experiment(projname, session, operation, username, ts, success):
    experiment = experiments(projname, session,
                             operation, username, ts, success)
...
def create_xdebug(expid, selcmdctr, httpreqctr, name, content):
    xdebug = xdebugs(expid, selcmdctr, httpreqctr, name, content)
...
exp_id = create_experiment(args.projname, args.session, args.operation,
                           args.username, args.ts, args.success)
